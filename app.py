...
with st.form('prediction_form'):
    ...
    with left:
        ...
        km_driven = st.number_input('Kilometers driven', min_value=0.0, value=None, step=1000.0, placeholder='Enter kilometers driven')
        fuel = st.selectbox('Fuel type', ['Select fuel type', 'Diesel', 'Petrol', 'CNG', 'LPG', 'Electric'])
        seller_type = st.selectbox('Seller type', ['Select seller type', 'Individual', 'Dealer', 'Trustmark Dealer'])

    with right:
        transmission = st.selectbox('Transmission', ['Select transmission', 'Manual', 'Automatic'])
        owner = st.selectbox('Owner history', ['Select owner history', 'First Owner', 'Second Owner', 'Third Owner', 'Fourth & Above Owner', 'Test Drive Car'])
        mileage = st.number_input('Mileage (km/ltr/kg)', min_value=0.0, value=None, step=0.1, placeholder='Enter mileage')
        engine = st.number_input('Engine (CC)', min_value=0.0, value=None, step=50.0, placeholder='Enter engine capacity')
        max_power = st.number_input('Max power', min_value=0.0, value=None, step=1.0, placeholder='Enter max power')
        seats = st.number_input('Seats', min_value=1.0, max_value=10.0, value=None, step=1.0, placeholder='Enter number of seats')
